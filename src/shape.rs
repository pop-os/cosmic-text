                    ...
                    visual_line.ranges[range.clone()].iter()
                {
                    ...
                    for i in starting_word..ending_word + usize::from(ending_glyph != 0) {
                        ...
                        for glyph in included_glyphs {
                            let match_mono_em_width = match_mono_width.map(|w| w / glyph.font_size);
                            let glyph_font_size = match (
                                    ...
                                    if glyph_em_width != match_em_width =>
                                {
                                    ...
                                    let glyph_font_size = math::roundf(glyph_to_match_factor)
                                        ...
                                        * glyph.font_size;
                                    log::trace!("Adjusted glyph font size ({font_size} => {glyph_font_size})", font_size = glyph.font_size);
                                    glyph_font_size
                                }
                                _ => glyph.font_size,
                            };
                            ...
                            glyphs
                                .push(glyph.layout(
                                    /*glyph_font_size, */ x, y, x_advance, span.level,
                                ));
                            if !self.rtl {
                                x += x_advance;
                            }
