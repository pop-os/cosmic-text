...
impl<'a> TextBufferLine<'a> {
    ...
    pub fn layout(
        ...
        if self.layout_opt.is_none() {
            ...
            shape.layout(font_size, width, &mut layout, 0);
...
impl<'a> TextBuffer<'a> {
    pub fn new(font_matches: &'a FontMatches<'a>, metrics: TextMetrics) -> Self {
        let mut buffer = Self {
            font_matches,
            lines: Vec::new(),
            metrics,
    ...
    pub fn shape_until(&mut self, lines: i32) -> i32 {
        ...
        for line in self.lines.iter_mut() {
            ...
            let layout = line.layout(self.font_matches, self.metrics.font_size, self.width);
    ...
    fn relayout(&mut self) {
        ...
        for line in self.lines.iter_mut() {
            if line.shape_opt.is_some() {
                ...
                line.layout(self.font_matches, self.metrics.font_size, self.width);
            }
        }
    ...
    pub fn action(&mut self, action: TextAction) {
        match action {
            ...
            TextAction::Left => {
                let rtl_opt = self.lines[self.cursor.line.get()]
                    .shape_opt
                    .as_ref()
                    .map(|shape| shape.rtl);
            ...
            TextAction::Insert(character) => {
                if character.is_control() && !['\t', '\u{92}'].contains(&character) {
    ...
    fn click(&mut self, mouse_x: i32, mouse_y: i32) {
        ...
        for (line_i, line) in self.lines.iter().enumerate() {
            ...
            for layout_line in layout {
                ...
                if mouse_y >= line_y - font_size && mouse_y < line_y - font_size + line_height {
                    ...
                    'hit: for (glyph_i, glyph) in layout_line.glyphs.iter().enumerate() {
                        if mouse_x >= glyph.x as i32 && mouse_x <= (glyph.x + glyph.w) as i32 {
                            ...
                            for (egc_i, egc) in cluster.grapheme_indices(true) {
                                if mouse_x >= egc_x as i32 && mouse_x <= (egc_x + egc_w) as i32 {
                    ...
                    match layout_line.glyphs.get(new_cursor_glyph) {
                        ...
                        None => {
                            if let Some(glyph) = layout_line.glyphs.last() {
